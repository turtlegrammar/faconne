...
;; `g/transformer` creates a function from a domain 'schema' -- a map/vector/set
;; of symbols -- and a range 'schema'.

(def swap-key-order (g/transformer {k1 {k2 v}} {k2 {k1 v}}))

(swap-key-order {:a {:z 1 :y 2} :y {:a 6 :z 9}})
;; => {:z {:a 1, :y 9}, :y {:a 2}, :a {:y 6}}

;; concat can be written as its type signature: [[a]] -> [a]
(def concat' (g/transformer [[a]] [a]))
...
(def group-by-sum-of-dice (g/transformer {{:keys [name]} [roll1 roll2]}
...
(def by-composer-and-year (g/transformer [{:keys [composer year] :as piece}]
                                         {composer {year [piece]}}))
...
(defn filter' [p coll]
  (g/transform coll [a] [a] :where [(p a)]))
...
;;(def same-names (g/transform {{:name prof-name}
