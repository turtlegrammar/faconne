...
;;; Often when working with Clojure, you need to transform one nested collection
;;; into another. Faconne makes this as easy as visually specifying the input domain and
;;; output range. Here are some simple examples:
...
;;; what we really want is a map: artists -> titles -> stores -> quantity.
;;; To make it more contrived, we care only about records available in quantities greater than 2.
;;; This is straightforward with Faconne's map destructuring.
...
;;; Faconne is reasonably efficient. When iterating over sequences, it tends
;;; to be about comparable to handwritten loops. It's faster than threading through
;;; sequence fns like `partition` and `map`. When iterating over maps, Faconne
;;; tends to be faster since it produces less short-lived garbage.
...
;;; Faconne is not always faster; I've found that `(f/transformer [[a]] [a])` is about half as fast
;;; as `#(reduce into [] %)` due to administrative overhead. But the more convoluted the collection,
;;; the more efficient Faconne will be compared to idiomatic sequence-manipulating code since
;;; Faconne will generate less short-lived garbage.
